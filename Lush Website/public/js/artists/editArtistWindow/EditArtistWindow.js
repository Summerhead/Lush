...
export default class EditArtistWindow {
  constructor(editArtistWindowContainer, href) {
    ...
    this.configureEditArtistWindow();
    this.addUploadAction();
    this.displayEditArtistWindow();
  }

  configureEditArtistWindow() {
    this.closeButton.addEventListener("click", this.hideEditArtistWindow);

    window.onclick = (event) => {
      if (event.target.matches("#edit-artist-window-background")) {
        this.hideEditArtistWindow();
      }
    };
  ...
  displayEditArtistWindow() {
    document
      .getElementsByTagName("body")[0]
      .prepend(this.editArtistWindowContainer);
  }

  openEditArtistWindow(artistLi) {
